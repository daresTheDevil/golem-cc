...
## PHASE 0.5: UPDATE CHANGELOG

Before proceeding with the release, update CHANGELOG.md to move [Unreleased] entries to the new version:
...
if [ ! -f "CHANGELOG.md" ]; then
  ...
  if grep -q "^### " CHANGELOG.md && grep -B20 "^## \[Unreleased\]" CHANGELOG.md | grep -q "^- "; then
    # Use Node.js to update changelog (golem changelog lib)
    node -e "
      const changelog = require(process.env.GOLEM_HOME + '/lib/changelog.js');
      const fs = require('fs');
      const content = fs.readFileSync('CHANGELOG.md', 'utf-8');
      const ast = changelog.parseChangelog(content);
      const VERSION = '$NEW_VERSION'; // Set by release script
      const TODAY = new Date().toISOString().split('T')[0];
      const released = changelog.releaseVersion(ast, VERSION, TODAY);
      if (released) {
        const remote = changelog.getGitRemote();
        if (remote) {
          released.links = changelog.generateComparisonLinks(released, remote);
        }
        fs.writeFileSync('CHANGELOG.md', changelog.renderChangelog(released));
        console.log('✓ Updated CHANGELOG.md for v' + VERSION);
      } else {
        console.log('ℹ No unreleased changes to publish');
      }
    "
    ...
    echo "ℹ No unreleased changes in CHANGELOG.md"
  fi
fi
```

If CHANGELOG.md doesn't exist or [Unreleased] is empty, that's OK — just note it and continue.
