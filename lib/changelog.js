...
function parseChangelog(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Handle multiple spaces around dash
 ...
 */
function showUsage() {
  return `Usage: golem changelog <category> <message>
...
Examples:
  golem changelog added "New feature X"
  golem changelog fixed "Bug in authentication"
  golem changelog security "Fixed XSS vulnerability"
  golem changelog --dry-run added "Preview mode"

Format: Keep a Changelog v1.1.0 (https://keepachangelog.com)`;
}
 ...
 */
function run(args) {
  if (!Array.isArray(args) || args.length === 0 || args.includes('--help') || args.includes('-h')) {
    return {
      success: args.includes('--help') || args.includes('-h'),
      message: showUsage(),
    };
  }
